...
Создание экземпляра класса File не приводит к созданию каталога на жестком диске компьютера. Для работы с каталогами необходимо вызывать методы у объекта File, например:
- создание - mkdir();
...
Класс FileOutputStream предназначен для записи байтов в файл. Он является производным от класса OutputStream, поэтому наследует всю его
функциональность. Через конструктор класса FileOutputStream задается файл, в который производится запись. Класс поддерживает несколько конструкторов:
```java
...
Первый способ закрыть поток и освободить ресурсы заключается в использовании
комбинаций блоков try-catch-finally. Например, считаем данные из файла:
```java
